...
( function() {
	...
	jQuery.extend( support, {
		...
		reliableMarginRight: function() {
			...
			// margin-right
			// This support function is only executed once so no memoizing is
			// needed.
			var ret,
	...
	simulate: function( type, elem, event ) {
		var e = jQuery.extend(
			...
			event,
			{
				...
				// for which stopPropagation() is noop, so there is no need for
				// that anymore.
				//
				// For the 1.x branch though, guard for "click" and "submit"
				// events is still used, but was moved to
				// jQuery.event.stopPropagation function
				// because `originalEvent` should point to the original event
				// for the constancy
